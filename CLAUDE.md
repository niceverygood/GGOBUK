...
- **2026-08-16**: **DB 운영 CLI(`scripts/db.mjs`) + 라이브 불변식 검증.** `pnpm db:status`(적용 실측)·`db:verify`(라이브 보안 불변식 14건)·`db:apply`·`db:query`(읽기전용). 정적 테스트는 **SQL 파일**만 보므로 "파일은 옳은데 DB엔 안 들어감"(2026-06-10 드리프트)을 못 잡는다 — `db:verify` 가 그 간극을 메운다(users 광범위 UPDATE 차단·credit_balance/is_admin 수정 불가·usage_logs 변조 차단·open_bread 인자 1개·increment_chat_usage KST+호출자 검증·대표프로필 소유권 FK 등, 프로덕션 14/14 통과). ⚠️ **같은 날 사고**: 안내 코드블록에 `--yes` 가 들어 있어 승인 대기였던 migration 22(프로필 중복 정리)가 붙여넣기로 적용됨. 결과는 preflight 예측과 일치(soft delete 17행, 유료 풀이 손실 0, 대표프로필 재지정 완료)라 유지. 재발 방지로 **데이터 변경 마이그레이션은 플래그로 적용 불가** — top-level UPDATE/DELETE 또는 `🔴 승인` 표시가 있으면 번호를 **손으로 타이핑**해야 통과하고 비대화형에선 fail-closed. 함수 본문(`$$…$$`) 안의 UPDATE 는 세지 않는다(오탐 → 경고 피로 → 게이트 무력화 방지). 회귀 테스트 `migration-apply-gate.test.ts`. 상세는 `docs/parity-rebuild/DECISIONS.md` D-14.
