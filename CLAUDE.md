...
- **2026-06-26**: **카카오톡 채널 챗봇 응대 — 카카오 i 오픈빌더 스킬 서버.** 헬로우봇처럼 카톡 채널 안에서 꼬북점이 직접 사주 상담하도록 스킬 서버 엔드포인트 신설(`POST /api/kakao/skill`). 카톡 채널 사용자는 앱 회원이 아닌 **익명 봇 사용자**라 `userRequest.user.id`(채널·봇 스코프 고유키)를 PK 로 새 테이블 `kakao_chat_users`(mig 18)에 생년월일·성별·페르소나·최근 대화(history jsonb, 최근 6턴) 보관. RLS enable + 정책 0개 → admin(service_role)만 접근. **흐름**: ① 생년월일 미보유 시 자유 발화에서 출생정보 파싱(`extractBirthInfo`, cheap/haiku JSON 추출 — 한국어 날짜·오전/오후·음력/윤달·성별을 견고하게) → 부족하면 되묻기. ② 받는 즉시 `buildSajuResult` 계산·저장 후 **LLM 없이** 일주 60갑자 데이터로 환영 카드 즉답(`iljuProfileOf`) + 주제 퀵리플라이(오늘운세/연애/금전/진로/총운) + `/ilju/[slug]` 공유 링크(루프① 재점화). ③ 이후 발화는 페르소나 사주 상담 → `buildChatSystem`(chat.ts 에서 추출·export, 웹 채팅과 동일 프롬프트 공유) + `complete`(saju tier). **5초 제한 대응**: LLM 답은 카카오 **콜백 모드**로 — 1차로 `useCallback:true`+"생각 중" 임시메시지 응답하고, `after()` 백그라운드에서 LLM 호출 후 `callbackUrl` 로 최종답 POST. 콜백 미설정 블록은 동기 폴백(짧게 시도). **세부**: cite 마크업(`[[위치:글자]]`)은 카톡엔 카드 변환이 없어 글자만 남기고 제거, simpleText 1000자 한도라 문단/문장 경계로 최대 3버블 분할(`chunkText`), 페르소나 전환 발화("도사로 바꿔줘") + "정보 변경"/"리셋" 명령, 비용 가드 일일 메시지 캡(`KAKAO_DAILY_MSG_LIMIT`, 기본 30 — 초과 시 앱 유도). **보안**: 카카오는 스킬 요청에 서명을 안 붙이므로 오픈빌더 스킬 URL 에 비밀(`KAKAO_SKILL_SECRET`)을 헤더(`X-Skill-Secret`) 또는 쿼리(`?secret=`)로 심어 가드 — 미설정이면 셋업 편의로 통과(경고 로그, 출시 전 설정 필요). 오류 시에도 항상 200+유효 템플릿 반환해 "오류" 말풍선 회피. 파일: `src/lib/kakao/{skill,birthinfo,chatbot}.ts` + 라우트 + `db.ts` 타입 + env 예시. tsc/eslint green, skill 빌더 유닛테스트 5종 green, 전체 스위트 11 green. **운영 메모(사람)**: 카카오 비즈니스 채널 개설 → 오픈빌더 봇 생성 → 폴백 블록에 스킬 연결(콜백 사용 ON) + 스킬 URL 에 `?secret=` → 채널 홈 버튼(연애운/궁합 등)은 메시지 액션으로 매핑. **한계**: 카톡 채널 사용자는 앱 계정과 미연동(별도 익명 ID) — 추후 카카오싱크/딥링크로 계정 연결 가능.
...
- ~~Capacitor wrap for native iOS/Android~~ → 완료(2026-06-09~10): iOS·Android 모두 Capacitor(원격로드) + 직접 StoreKit/Play Billing IAP(`/api/payment/iap/verify`). 상세는 Decision Log 참조.
