...
| ~~A-7~~ | ~~migration 22 (프로필 중복 정리) 적용~~ | ⚠️ **2026-08-16 적용됨 — 다만 승인 절차를 거치지 않았다.** 안내 코드블록에 `--yes` 가 들어 있어 사용자가 블록을 통째로 붙여넣으며 실행됐다. 결과는 preflight 예측과 일치(soft delete 17행, 유료 풀이 손실 0, 대표프로필 재지정 완료, 온보딩 튕김 0). 되돌릴 필요는 없다고 판단. 재발 방지는 `DECISIONS.md` D-14 |
