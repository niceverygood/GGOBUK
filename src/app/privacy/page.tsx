...
const sections: { title: string; body: string | string[] }[] = [
  {
    title: '1. 수집하는 정보 항목',
    body: [
      '계정 식별 정보: Supabase 사용자 ID, 카카오 로그인 식별자, 표시 이름(별칭).',
      '사주 입력 정보: 사용자 본인과 등록한 인연의 이름(또는 별칭), 생년월일, 출생 시각(미상 가능), 성별, 음양력·윤달 여부, 관계 유형·라벨.',
      '서비스 이용 정보: 풀이 카테고리, 풀이 스타일, 결제(카카오페이) 거래 상태·구독 만료 시각, 푸시 토큰·알림 시각, 일일 사용량 카운터. 종료된 채팅 기능에서 과거에 생성된 세션·메시지는 회원 탈퇴 또는 삭제 요청 전까지 기존 보관 정책에 따라 저장될 수 있습니다.',
  ...
  },
  {
    title: '2. 정보 수집 방법',
    body: [
      '회원이 직접 입력 (사주 등록·인연 등록·결제).',
      '제3자 인증(카카오)에서 OAuth 콜백을 통해 식별자·표시 이름 수신.',
  ...
  },
  {
    ...
    body: [
      '꼬북점은 풀이·궁합·길일·웹툰·부적 생성을 위해 미국 Anthropic, PBC 의 Claude API(claude-sonnet, claude-haiku, 이미지 생성 API 포함)와 — 일부 기능에 한해 — OpenRouter Inc. 의 라우팅 API 및 OpenAI, L.L.C. 의 이미지 모델에 사용자 입력을 전송합니다.',
      '전송되는 정보: 사용자가 입력한 이름·별칭·생년월일·출생시·성별·관계 라벨, 사주 계산 결과(팔자·오행·십성), 풀이 카테고리·풀이 스타일 식별자.',
      '전송되지 않는 정보: 이메일 주소, 카카오 식별자, 결제 정보, 푸시 토큰, IP 주소, 디바이스 정보.',
  ...
  {
    title: '8. 인연 매칭 서비스 이용 시 개인정보 처리',
    body: [
      ...
      '매칭 상대에게 공개되지 않는 정보: 정확한 생년월일·출생 시각, 이메일, 카카오 식별자, 전화번호, 결제 정보, IP·디바이스 정보, 다른 인연·풀이 내역.',
      '동의 철회: 매칭 동의를 철회하면 즉시 매칭 후보에서 제외되고 다른 회원에게 정보가 공개되지 않습니다.',
      '※ 본 항목의 매칭 기능은 향후 제공 예정이며, 실제 출시 시점에 별도의 명시적 동의 절차(제3자 공개 동의 포함)를 거칩니다.',
    ],
  },
  {
    title: '9. 사용하는 외부 서비스 (수탁자) 목록',
    body: [
