...
export const INTERPRETATION_CATEGORIES: Array<{
  ...
  },
  {
    ...
    anchors: ['일주 60갑자 특성', '일지가 배우자궁으로서 갖는 의미', '일간 통근', '도화·천을귀인 매칭'],
  },
  {
    key: 'strength',
    title: '타고난 장점',
  ...
  },
  {
    ...
    anchors: ['일주', '월지', '천간 투출', '원국 내 합/충', '음양 비율'],
  },
  {
    key: 'career',
    title: '직업과 적성',
  ...
  },
  {
    ...
    anchors: ['일지', '배우자성(재성/관성)', '도화/천을귀인', '일간 강약 + 배우자성 비율'],
  },
  {
    key: 'family',
    title: '가족 관계',
    ...
    anchors: ['연주(부모궁)', '인성(어머니·보호)', '편재(아버지)', '비겁(형제)', '시주(자녀궁)'],
  },
  {
    key: 'friends',
    title: '대인관계',
...
[근거 의무 — 매우 중요]
- 입력 컨텍스트의 "## 일간 강약", "## 격국", "## 용신 후보", "## 통근/투출", "## 원국 내 합·충·형·파·회", "## 진행 중 흐름" 섹션을 반드시 활용한다.
...
- 첫 줄은 반드시 "한 줄 결론:"으로 시작. 2-3문장으로 핵심을 잡는다.
- 그 다음 정확히 4개 섹션을 이 순서로 작성한다.
  1) "## 판독 근거 표" — 4행 이상의 마크다운 표. 열은 정확히 "사주 근거 | 작용 방식 | 현실 체감" 세 개. 각 셀 25–40자.
  2) "## 체감 체크포인트" — 사용자가 스스로 맞춰볼 수 있는 4-6개 bullet. 각 줄은 "이럴 때 자주 ~함" 형식.
  3) "## 깊은 풀이" — 3-4개 단락. 각 단락은 (근거 → 작용 방식 → 현실 장면 → 처방)이 한 흐름으로 이어진다. 단락당 4-6문장.
  4) "## 활용 처방" — 4행 이상의 마크다운 표. 열은 정확히 "상황 | 조심할 점 | 써먹는 법" 세 개. 각 셀 25–40자.

[표현 규칙]
- 명리 술어는 처음 등장할 때 한글 뜻 병기. 예: 식신(食神, 표현과 생산성).
- "정확하다"고 주장하지 말고, 계산된 근거와 현실 장면을 촘촘히 연결해 사용자가 체감하도록 만든다.
- 전체 분량은 2,000-2,800자 안팎. 토큰이 모자라더라도 4개 섹션 구조는 절대 깨지 않는다.`;
