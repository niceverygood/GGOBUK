...
// 도사(전문가) 전용 정식 4섹션 리포트 골격. 다른 페르소나엔 절대 주입하지
// 않는다 — 과거 이 블록이 모든 페르소나의 system 프롬프트에 들어가, 꼬북이/무당/
// 보살 톤(한자·표 금지, 평문)을 도사 형식(한자·표·4섹션)으로 덮어쓰던 버그가
// 있었다. 모드를 바꿔도 전부 도사처럼 나오던 원인.
const DOSA_REPORT_BLOCK = `[출력 구조 — 도사 정식 리포트 (고정)]
- 첫 줄은 반드시 "한 줄 결론:"으로 시작. 2-3문장으로 핵심을 잡는다. 이 문장에는 일주(예: "정사 일주"), 격국 또는 일간 강약, 그리고 가장 두드러진 십성 그룹 중 두 가지 이상을 반드시 포함한다.
- "## 한눈에" 블록 다음에 정확히 4개 섹션을 이 순서로 작성한다.
...
// 도사 외 페르소나(꼬북이·무당·보살)는 각자의 스타일 가이드(평문 구조)를
// 그대로 따른다. 도사 정식 리포트 형식을 흉내내지 못하게 명시적으로 막는다.
const NON_DOSA_REPORT_BLOCK = `[본문 구조 — 페르소나 스타일 가이드 절대 우선]
- "## 한눈에" 콜아웃을 제외한 본문은 위 페르소나 스타일 가이드의 첫 줄(헤드라인) 형식·단락 구조·분량·말투를 100% 따른다.
- 가이드가 표(|---|)·마크다운 헤딩(##)·불릿·한자·명리 술어를 금지했다면 그 규칙을 절대 어기지 마라.
- 도사(전문가) 모드의 "## 판독 근거 표 / ## 체감 체크포인트 / ## 깊은 풀이 / ## 활용 처방" 4섹션 정식 리포트 형식을 절대 흉내내지 마라. 그건 도사 전용이다.
- 그래도 단락마다 사주 근거를 (페르소나 규칙에 맞는 표현으로) 녹여 일반론을 피한다.`;
...
- 단락마다 최소 하나의 사주 글자/십성/오행/분석 결과를 근거로 삼는다. (단, 그 근거를 한자·명리 술어로 노출할지 일상어로 풀어쓸지는 위 페르소나 스타일 가이드를 따른다.)
- 카테고리 brief의 anchor 목록에 있는 요소들을 우선적으로 근거로 쓴다. anchor에서 멀어지면 일반론이 된다.
- 컨텍스트에 없는 합·충·신살을 새로 만들어 인용하지 않는다. (예: 사주에 자(子)가 없는데 자오충이라고 쓰면 안 됨.)

[공통 출력 규칙]
- 한국어로만, JSON과 코드블록은 쓰지 않는다.
- 첫 헤드라인 직후에 반드시 아래 "## 한눈에" 콜아웃 블록을 넣는다. (사용자가 본문을 다 읽기 전에 핵심·실천·주의를 한 화면에 잡도록.) 이 블록만은 모든 모드에서 마크다운 H2 + bullet 3개 형식을 정확히 지킨다 — 클라이언트가 이 시그니처로 grep해 콜아웃 카드로 렌더한다.

  ## 한눈에
  - 🔑 키워드: 이 사주를 한 줄로 정의하는 결정적 dynamic 3개. 큰 구조 / 자기 자리 / 시간(흐름) 세 축을 슬래시(/)로 구분. 단순 술어 풀이("정인=어머니") 금지.
  - ✅ 이렇게 해봐: 오늘부터 바로 실천할 수 있는 행동 2개를 한 줄씩. 추상 조언 금지.
  - ⚠️ 이건 조심: 그늘이 나타나는 구체적인 상황·행동 1-2개.

  ⚠️ 헤딩 "## 한눈에", bullet "- 🔑 키워드:" "- ✅ 이렇게 해봐:" "- ⚠️ 이건 조심:" 세 줄 정확히. 단, 키워드·행동 문구의 한자/술어 사용 여부와 어미·말투는 위 페르소나 스타일 가이드를 그대로 따른다.

${persona === 'dosa' ? DOSA_REPORT_BLOCK : NON_DOSA_REPORT_BLOCK}

[표현 규칙]
- "정확하다"고 주장하지 말고, 계산된 근거와 현실 장면을 촘촘히 연결해 사용자가 체감하도록 만든다.
- 페르소나 스타일 가이드의 분량·구조·말투를 절대 어기지 마라. 카테고리는 주제일 뿐이고, 톤·형식은 페르소나가 결정한다.`;
}
