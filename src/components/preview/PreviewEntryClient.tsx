...
export function PreviewEntryClient() {
  ...
  return (
    <main className="relative mx-auto min-h-dvh w-full max-w-md overflow-x-hidden px-5 pb-32 pt-10">
      <div className="hanji-overlay" />
      <div className="relative">
        <Badge>미리보기 모드 · 로그인 없음</Badge>
        <h1 className="mt-3 text-2xl font-black leading-snug tracking-tight text-navy">
          사주를 넣으면
          <br />
          등껍질 전체를 바로 펼쳐볼게요
        </h1>

        <div className="mb-5 mt-6 flex items-end gap-3">
        ...
        <Card className="space-y-5 p-5">
          <Field label="이름 또는 닉네임">
            <input
              ...
              className="h-12 w-full rounded-2xl border border-navy/10 bg-white px-4 text-sm font-bold text-navy focus:outline-none focus:ring-2 focus:ring-mint"
            />
          </Field>
        ...
        <p className="mt-4 text-center text-xs font-bold text-muted">
          기본값 그대로 진행하면 1985-11-14 14:05 남성 기준으로 계산돼.
        </p>
      </div>

      <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-ivory via-ivory/95 to-transparent px-5 pb-6 pt-3">
...
function Field({
  ...
  return (
    <div>
      <p className="mb-2 ml-1 text-xs font-extrabold text-muted">{label}</p>
      {children}
    </div>
  );
}
